...
bool connectFour_X_O_Board::update_board (int x,int y, char mark){ //y is ignored because we just need the column idx
...
bool connectFour_X_O_Board::is_winner() {
    ...
    for (int i=0;i<n_rows; ++i) {
        ...
        if (board[i][n_cols-1]==b&&tmp==4&&b!=' '&&b!='\0') //checking for the last 4 elements in the row ( - - - * * * * )
            return true;
        else
            tmp=0;
    }
...
int connectFour_X_O_Board::gameID() {
    return 0;
}

char **connectFour_X_O_Board::getBoard() { // Used in AI
    return board;
}

int *connectFour_X_O_Board::getCol() { // Used in AI
    return col;
}

void connectFour_X_O_Board::undoPlay(int x) { //Used in AI
    ++col[x];
    board[col[x]][x]=' ';
    --n_moves;
}
